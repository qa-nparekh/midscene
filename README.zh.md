...
|  使用 SQAI mcp 的方法，浏览页面（ https://www.saucedemo.com/ ），进行登录，添加商品、下单商品最终根据 mcp 执行的步骤和 playwright example 生成最终的测试用例   | <video src="https://github.com/user-attachments/assets/5cab578d-feb3-4250-8c7e-6793fe38a5be" height="300" />        |

## 💡 特性

### 用自然语言编写自动化脚本
- 描述你的目标和步骤，SQAI 会为你规划和操作用户界面。
- 使用 Javascript SDK 或 YAML 格式编写自动化脚本。

### Web & Mobile App & 任意界面
- **Web 自动化 🖥️**: 可以[与 Puppeteer 集成](https://sqai.tech/integrate-with-puppeteer.html)，[与 Playwright 集成](https://sqai.tech/integrate-with-playwright.html)或使用[桥接模式](https://sqai.tech/bridge-mode-by-chrome-extension.html)来控制桌面浏览器。
- **Android 自动化 📱**: 使用 [Javascript SDK](https://sqai.tech/integrate-with-android.html) 配合 adb 来控制本地 Android 设备。
- **iOS 自动化 🍎**: 使用 [Javascript SDK](https://sqai.tech/zh/integrate-with-ios.html) 配合 WebDriverAgent 来控制本地 iOS 设备。
- **任意界面自动化 🌐**: 使用 [Javascript SDK](https://sqai.tech/integrate-with-any-interface.html) 来控制你自己的界面。

### 工具
- **用于调试的可视化报告**: 通过我们的测试报告和 Playground，你可以轻松理解、回放和调试整个过程。
- [**使用缓存，提高执行效率**](https://SQAIjs.com/zh/caching.html): 使用缓存能力重放脚本，提高执行效率。
- **MCP**: 允许其他 MCP Client 直接使用 SQAI 的能力。[**Web MCP**](https://SQAIjs.com/zh/web-mcp.html) [**Android MCP**](https://SQAIjs.com/zh/mcp-android.html)。

### 三种类型的 API
  - [**交互 API 🔗**](https://SQAIjs.com/zh/api.html#interaction-methods): 与用户界面交互。
  - [**数据提取 API 🔗**](https://SQAIjs.com/zh/api.html#data-extraction): 从用户界面和 DOM 中提取数据。
  - [**实用 API 🔗**](https://SQAIjs.com/zh/api.html#more-apis): 实用函数，如 `aiAssert()` （断言）, `aiLocate()` （定位）, `aiWaitFor()` （等待）。

## 👉 零代码快速体验

- **[Chrome 插件](https://sqai.tech/zh/quick-experience.html)**: 通过 [Chrome 插件](https://sqai.tech/zh/quick-experience.html) 立即开始体验，无需编写代码。
- **[Android Playground](https://sqai.tech/zh/quick-experience-with-android.html)**: 内置的 Android Playground 可以控制你的本地 Android 设备。
- **[iOS Playground](https://sqai.tech/zh/quick-experience-with-ios.html)**: 内置的 iOS Playground 可以控制你的本地 iOS 设备。

## ✨ 视觉语言模型驱动

SQAI 支持视觉语言模型，例如 `Qwen3-VL`、`Doubao-1.6-vision`、`gemini-2.5-pro` 和 `UI-TARS`。
...
更多信息请查看 [选择 AI 模型](https://SQAIjs.com/zh/choose-a-model)。

## 💡 两种风格的自动化

### 自动规划

SQAI 会自动规划步骤并执行。它可能较慢，并且深度依赖于 AI 模型的质量。
...
](https://sqai.tech/zh/blog-programming-practice-using-structured-api.html)

## 👀 与其它工具比较

* **视觉驱动带来可靠性和效率**: 借助视觉语言模型，SQAI 适用于 Web 和移动 App 的自动化，无论界面采用何种技术栈。

* **调试体验**: 你很快就会发现，调试和维护自动化脚本才是真正的痛点。无论模型多么强大，你仍然需要调试过程以确保其保持长期稳定。SQAI 提供了可视化报告、内置的 Playground 和 Chrome 插件，以调试整个运行过程。这是大多数开发者真正需要的能力。

* **开源、免费、部署灵活**: SQAI 是一个开源项目，并且支持自托管模型。

* **与 Javascript 集成**: 你可以永远相信 Javascript 😎

## 📄 资源

* 官网和文档: [https://sqai.tech](https://sqai.tech/zh)
* 示例项目: [https://github.com/qa-nparekh/sqai-examples](https://github.com/qa-nparekh/sqai-examples)
* API 文档: [https://sqai.tech/zh/api.html](https://sqai.tech/zh/api.html)
* GitHub: [https://github.com/qa-nparekh/SQAI](https://github.com/qa-nparekh/SQAI)## 🤝 社区
...
* [SQAI-ios](https://github.com/lhuanyu/SQAI-ios) - iOS 设备自动化工具
* [SQAI-pc](https://github.com/Mofangbao/SQAI-pc) - 支持 Windows、macOS 和 Linux 的 PC 操作设备
* [SQAI-pc-docker](https://github.com/Mofangbao/SQAI-pc-docker) - 预装 SQAI-PC 服务器的 Docker 容器镜像
* [SQAI-Python](https://github.com/Python51888/SQAI-Python) - Python 版本的 SQAI SDK
* [SQAI-java](https://github.com/Master-Frank/SQAI-java) - Java 版本的 SQAI SDK，便于在 JVM 项目中使用自动化能力
...
SQAI 遵循 [MIT 许可协议](https://github.com/qa-nparekh/SQAI/blob/main/LICENSE)。
