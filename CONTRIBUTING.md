# SQAI Contribution Guide

Thanks for showing interest in contributing to SQAI. Before starting your contribution, please take a moment to read the following guidelines.
...
If you see `REPLACE_ME_WITH_REPORT_HTML` in the report file, it's usually because of the circular dependency issue of SQAI. You can rebuild the entire project without nx cache to solve this issue.
...
SQAI_MODEL_NAME="gpt-4o-2024-08-06"
```
...
```sh
npx nx test @SQAI/web
# Test with AI-related features, it will need to create a .env file
npx nx test:ai @SQAI/web
```
...
All SQAI packages will use a fixed unified version.

The release notes are automatically generated by [GitHub releases](https://github.com/qa-nparekh/SQAI/releases).
...
1. [Run the release action](https://github.com/qa-nparekh/SQAI/actions/workflows/release.yml).
2. [Generate the release notes](https://github.com/qa-nparekh/SQAI/releases).
...
- Select the `apps/chrome-extension/extension_output/SQAI-extension-v{version}.zip` file
...
Due to some issues with circular dependencies, you need to execute the full build process within the entire repository to compile the SQAI project, rather than compiling the ⁠@SQAI/core package separately.