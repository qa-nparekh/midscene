...
```typescript
import { agentFromAdbDevice, getConnectedDevices } from '@sqaitech/android';
...
You can use the `SQAI_ADB_PATH` environment variable to specify the path to the adb executable, `SQAI_ADB_REMOTE_HOST` environment variable to specify the remote adb host, `SQAI_ADB_REMOTE_PORT` environment variable to specify the remote adb port.
