...
```ini filename=.env
SQAI_MODEL_BASE_URL="replace with your model service URL/v1"
...
Install `@sqaitech/cli` globally (recommended for first-time users):
...
`@sqaitech/cli` supports glob patterns to batch-execute scripts, which is a shorthand for the `--files` argument.
