...
Playground is the fastest way to validate the connection and observe AI-driven steps without writing code. It shares the same core as `@sqaitech/ios`, so anything that works here will behave the same once scripted.
...
```bash
npx --yes @sqaitech/ios-playground
```
...
<PackageManagerTabs command="install @sqaitech/ios --save-dev" />
