...
`@sqaitech/web-bridge-mcp` exposes the [Chrome extension Bridge Mode](./bridge-mode) as an MCP service.
...
Add the Midscene Web Bridge MCP server (`@sqaitech/web-bridge-mcp`) in your MCP client. For model configuration parameters, see [Model strategy](./model-strategy).
...
```json
{
  "mcpServers": {
    "midscene-web": {
      "command": "npx",
      "args": ["-y", "@sqaitech/web-bridge-mcp"],
      "env": {
        "SQAI_MODEL_BASE_URL": "replace with your model service URL",
        "SQAI_MODEL_API_KEY": "replace with your API Key",
        "SQAI_MODEL_NAME": "replace with your model name",
        "SQAI_MODEL_FAMILY": "replace with your model family",
...
Add the Midscene iOS MCP server (`@sqaitech/ios-mcp`) in your MCP client. For model configuration parameters, see [Model strategy](./model-strategy).
...
Add the Midscene Android MCP server (`@sqaitech/android-mcp`) in your MCP client. For model configuration parameters, see [Model strategy](./model-strategy).
...
`@sqaitech/computer-mcp` exposes the computer desktop automation capabilities as an MCP service, allowing AI to control your computer through mouse, keyboard, and screenshot operations.
...
Add the Midscene Computer MCP server (`@sqaitech/computer-mcp`) in your MCP client. For model configuration parameters, see [Model strategy](./model-strategy).
...
```typescript
import { mcpKitForAgent } from '@sqaitech/web/mcp-server';
...
**iOS platform**
```typescript
import { mcpKitForAgent } from '@sqaitech/ios/mcp-server';
```

**Android platform**
```typescript
import { mcpKitForAgent } from '@sqaitech/android/mcp-server';
```

**Computer platform**
```typescript
import { mcpKitForAgent } from '@sqaitech/computer/mcp-server';
```
