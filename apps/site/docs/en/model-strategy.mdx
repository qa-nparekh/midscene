...
2. Ensure the `SQAI_MODEL_FAMILY` environment variable is set correctly; otherwise element localization can drift significantly.
...
Set `DEBUG=sqai:ai:profile:stats` to print cost and latency information. You can also review token usage in the generated report files.

### "SQAI_MODEL_FAMILY is required" error

If you see "No visual language model (VL model) detected" or "SQAI_MODEL_FAMILY is required," make sure the `SQAI_MODEL_FAMILY` environment variable for your VL model is set correctly.

Starting with version 1.0, Midscene recommends using `SQAI_MODEL_FAMILY` to specify the vision model type. Legacy `MIDSCENE_USE_...` configs remain compatible but are deprecated.
