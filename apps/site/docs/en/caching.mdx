...
Configuration via `SQAI_CACHE=1` environment variable with cacheId, equivalent to read-write mode.

```javascript
// Old way, requires SQAI_CACHE=1 environment variable and cacheId
...
```bash
SQAI_CACHE=1 tsx demo.ts
```

## Using Playwright AI Fixture from `@sqaitech/web/playwright`

When using `PlaywrightAiFixture` from `@sqaitech/web/playwright`, pass the same `cache` options to control caching behaviour.
...
5. **Legacy approach**: Set `cacheId` and enable `SQAI_CACHE=1` environment variable
...
You can set the `DEBUG=sqai:cache:*` environment variable to get more debug logs for caching.
