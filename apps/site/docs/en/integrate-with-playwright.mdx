...
<PackageManagerTabs command="install @sqai/web playwright @playwright/test tsx --save-dev" />
...
import { PlaywrightAgent } from '@sqai/web/playwright';
...
<PackageManagerTabs command="install @sqai/web --save-dev" />
...
export default defineConfig({
  testDir: './e2e',
+ timeout: 90 * 1000,
+ reporter: [["list"], ["@sqai/web/playwright-reporter", { type: "merged" }]], // type optional, default is "merged", means multiple test cases generate one report, optional value is "separate", means one report for each test case
});
```
