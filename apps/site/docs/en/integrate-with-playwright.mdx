...
<PackageManagerTabs command="install @sqaitech/web playwright @playwright/test tsx --save-dev" />
...
import { PlaywrightAgent } from '@sqaitech/web/playwright';
...
<PackageManagerTabs command="install @sqaitech/web --save-dev" />
...
export default defineConfig({
  testDir: './e2e',
+ timeout: 90 * 1000,
+ reporter: [["list"], ["@sqaitech/web/playwright-reporter", { type: "merged" }]], // type optional, default is "merged", means multiple test cases generate one report, optional value is "separate", means one report for each test case
});
```
...
<PackageManagerTabs command="install playwright @playwright/test @sqaitech/web --save-dev" />
