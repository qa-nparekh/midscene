...
Each platform package (`@sqaitech/android`, `@sqaitech/ios`, `@sqaitech/web`, etc.) now exposes an independent CLI entry point, which is the foundation that Skills is built upon.
...
New `@sqaitech/computer-mcp` package provides PC desktop automation as a standalone MCP service. Developers can use desktop automation capabilities directly in MCP-compatible tools like Cursor and Trae without additional integration.
...
- **Dynamic configuration**: Fixed Playground not reading the `SQAI_REPLANNING_CYCLE_LIMIT` environment variable properly
...
- Renamed `OPENAI_API_KEY` → `SQAI_MODEL_API_KEY` (new variable preferred, old variable as fallback)
- Renamed `OPENAI_BASE_URL` → `SQAI_MODEL_BASE_URL` (new variable preferred, old variable as fallback)

### ⬆️ Upgrade to the latest version

Upgrade only the packages you use. For example:

`npm install @sqaitech/web@latest`
`npm install @sqaitech/android@latest`
`npm install @sqaitech/ios@latest`

If you use the CLI installed globally:

`npm i -g @sqaitech/cli`
...
- Set the `SQAI_REPLANNING_CYCLE_LIMIT` environment variable to customize the maximum number of re-planning cycles allowed when executing operations (aiAct).
...
```bash
export SQAI_REPLANNING_CYCLE_LIMIT=10 # default value is 10
```
...
```bash
SQAI_OPENAI_INIT_CONFIG_JSON='{"baseURL":"https://xxx.net/api/v3","apiKey":"xxx"}'
SQAI_MODEL_NAME='ep-20240925111815-mpfz8'
...
# Enable cache
+ SQAI_CACHE=true playwright test --config=playwright.config.ts
```
...
npx @sqaitech/cli --headed --url https://www.baidu.com --action "input 'weather', press enter" --sleep 3000

# Visit GitHub status page and save the status to ./status.json
npx @sqaitech/cli --url https://www.githubstatus.com/ \
