...
Playground is the fastest way to validate the connection and observe AI-driven steps without writing code. It shares the same core as `@sqaitech/android`, so anything that works here will behave the same once scripted.
...
```bash
npx --yes @sqaitech/android-playground
```
...
<PackageManagerTabs command="install @sqaitech/android --save-dev" />
...
import { AndroidAgent, AndroidDevice, getConnectedDevices } from '@sqaitech/android';
...
Merged reports are stored inside `sqai_run/report` by default. Override the directory with `SQAI_RUN_DIR` when running in CI.
...
```bash
export SQAI_ADB_PATH=/path/to/adb
export SQAI_ADB_REMOTE_HOST=192.168.1.100
export SQAI_ADB_REMOTE_PORT=5037
```
