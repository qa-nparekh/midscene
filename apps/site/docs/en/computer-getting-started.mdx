...
Playground is the fastest way to validate the connection and observe AI-driven steps without writing code. It shares the same core as `@sqaitech/computer`, so anything that works here will behave the same once scripted.
...
```bash
npx --yes @sqaitech/computer-playground
```
...
<PackageManagerTabs command="install @sqaitech/computer" />
...
```typescript
import { agentFromComputer } from '@sqaitech/computer';
...
```typescript
import { ComputerDevice, agentFromComputer } from '@sqaitech/computer';
...
```typescript
import { checkComputerEnvironment } from '@sqaitech/computer';
