...
缓存内容会保存到 `./sqai_run/cache` 目录下，以 `.cache.yaml` 为扩展名。

如果缓存未命中，SQAI 将会重新调用 AI 模型，并更新缓存文件。
...
通过环境变量 `SQAI_CACHE=1` 配合 cacheId 配置，等同于读写模式。

```javascript
// 旧方式，需要 SQAI_CACHE=1 环境变量和 cacheId
...
在使用 `@sqaitech/web/playwright` 中的 `PlaywrightAiFixture` 时，可以通过相同的 `cache` 配置来管理缓存行为。
...
5. **旧方式**: 设置了 `cacheId` 并启用了 `SQAI_CACHE=1` 环境变量
...
你可以删除 `./sqai_run/cache` 目录中的缓存文件，或者编辑缓存文件的内容。
...
在环境变量中配置 `DEBUG=sqai:cache:*`，你可以看到缓存相关的调试日志。
