...
SQAI_MODEL_API_KEY="...."
SQAI_MODEL_NAME="ep-..." # 来自火山引擎的推理接入点 ID 或模型名称
SQAI_MODEL_FAMILY="doubao-seed" # 也兼容 "doubao-vision"
```

### 千问 Qwen3.5 {#qwen35}

以[阿里云](https://www.aliyun.com/)的 `qwen3.5-plus` 模型为例，它的环境变量配置如下：
...
```bash
SQAI_MODEL_BASE_URL="https://open.bigmodel.cn/api/paas/v4" # 或 https://api.z.ai/api/paas/v4
SQAI_MODEL_API_KEY="......"
...
SQAI_MODEL_BASE_URL="https://open.bigmodel.cn/api/paas/v4" # 或 https://api.z.ai/api/paas/v4
SQAI_MODEL_API_KEY="......"
SQAI_MODEL_NAME="autoglm-phone" # 模型名以平台实际模型名为准
SQAI_MODEL_FAMILY="auto-glm" # 或 "auto-glm-multilingual"
```

**关于 `SQAI_MODEL_FAMILY` 配置**

AutoGLM 提供了两个版本的模型，通过 `SQAI_MODEL_FAMILY` 区分：
...
在 [Google Gemini](https://gemini.google.com/) 上申请 API 密钥后，可以使用以下配置。`SQAI_MODEL_NAME` 请填写你使用的 Gemini-3-Pro 或 Gemini-3-Flash 具体模型名：

```bash
SQAI_MODEL_BASE_URL="https://generativelanguage.googleapis.com/v1beta/openai/"
SQAI_MODEL_API_KEY="......"
SQAI_MODEL_NAME="gemini-3.0-pro" # 或 gemini-3-flash 的具体模型名
...
SQAI_MODEL_BASE_URL="https://ark.cn-beijing.volces.com/api/v3"
SQAI_MODEL_API_KEY="...."
SQAI_MODEL_NAME="ep-2025..." # 来自火山引擎的推理接入点 ID 或模型名称
SQAI_MODEL_FAMILY="vlm-ui-tars-doubao-1.5"
```

**关于 `SQAI_MODEL_FAMILY` 配置**

`SQAI_MODEL_FAMILY` 用于指定 UI-TARS 版本，使用以下值之一：
...
旧版本使用 `SQAI_USE_VLM_UI_TARS=DOUBAO` 或 `SQAI_USE_VLM_UI_TARS=1.5` 配置，该配置仍然兼容但已废弃，建议迁移到 `SQAI_MODEL_FAMILY`。
...
```bash
# 默认视觉模型：Qwen3-VL
export SQAI_MODEL_BASE_URL="https://..."       # Qwen3-VL 接口地址
export SQAI_MODEL_API_KEY="..."                # 你的 Qwen3-VL API Key
export SQAI_MODEL_NAME="qwen3-vl-plus"
export SQAI_MODEL_FAMILY="qwen3-vl"

# Planning 模型：GPT-5.1
export SQAI_PLANNING_MODEL_API_KEY="sk-..."    # 你的 GPT-5.1 API Key
export SQAI_PLANNING_MODEL_BASE_URL="https://..."
export SQAI_PLANNING_MODEL_NAME="gpt-5.1"

# Insight 模型：GPT-5.1
export SQAI_INSIGHT_MODEL_API_KEY="sk-..."     # 你的 GPT-5.1 API Key
