...
- `replanningCycleLimit: number`: `aiAct` 的最大重规划次数。默认值为 20（UI-TARS 模型默认 40）。推荐通过 agent 入参设置；`SQAI_REPLANNING_CYCLE_LIMIT` 环境变量仅作兼容读取。
...
const agent = new PuppeteerAgent(page, {
  modelConfig: {
    // 默认
    SQAI_MODEL_NAME: 'qwen3-vl-plus',
    SQAI_MODEL_API_KEY: 'sk-default-key',
    SQAI_MODEL_BASE_URL: '.....',
    SQAI_MODEL_FAMILY: 'qwen3-vl',

    // planning 意图
    SQAI_PLANNING_MODEL_NAME: 'gpt-5.1',
    SQAI_PLANNING_MODEL_API_KEY: 'sk-planning-key',
    SQAI_PLANNING_MODEL_BASE_URL: '...',

    // insight 意图
    SQAI_INSIGHT_MODEL_NAME: 'qwen-vl-plus',
    SQAI_INSIGHT_MODEL_API_KEY: 'sk-insight-key'
  }
});
```
...
- 参数：
  ...
  - `options?: Object` - 可选，一个配置对象，包含：
    ...
    - `deepThink?: 'unset' | true | false` - 当模型支持时（取决于 SQAI_MODEL_FAMILY），是否开启规划阶段的深度思考能力。默认值为 `'unset'`（等同于省略该参数），跟随模型服务商的默认策略。[详情参阅 deepThink 说明](./model-strategy#关于-aiact-方法的-deepthink-参数)。
...
```typescript
import { overrideAIConfig } from '@sqaitech/web/puppeteer'; // 或其他的 Agent

overrideAIConfig({
  SQAI_MODEL_BASE_URL: '...', // 推荐使用新的变量名
  SQAI_MODEL_API_KEY: '...', // 推荐使用新的变量名
  SQAI_MODEL_NAME: '...',
...
合并后的报告文件会保存在 `sqai_run/report` 目录下。你可以使用浏览器打开合并后的 HTML 文件查看所有自动化工作流的执行情况。
