...
* 千问系列: [Qwen3.5](./model-common-config#qwen35), [Qwen3-VL](./model-common-config#qwen3-vl), [Qwen2.5-VL](./model-common-config#qwen25-vl)
...
2. 确保模型的 `SQAI_MODEL_FAMILY` 环境变量配置正确，否则定位结果会出现明显偏移
...
`deepThink` 参数用于控制模型在规划时是否启用深度思考。目前支持 `qwen3-vl`、`qwen3.5`、`doubao-seed-1.6`、`doubao-seed-2.0`、`glm-v` 等系列模型。

启用后，你可以在报告中看到 `Reasoning` 栏目。开启后，规划耗时会增加，但结果会更准确。

**不同服务商的默认行为：**
- `qwen3.5` 在阿里云：默认开启（更慢，但更准确）
- `qwen3-vl` 在阿里云：默认关闭（更快，但准确性差）
- `doubao-seed-1.6` 在火山引擎：默认开启（更慢，但更准确）
- `doubao-seed-2.0` 在火山引擎：默认开启（更慢，但更准确）
...
在环境变量中设置 `DEBUG=sqai:ai:profile:stats`，即可打印模型的用量信息与响应时长。

你也可以在报告文件中查看模型的使用情况。

### "SQAI_MODEL_FAMILY is required" 错误

如果收到了 "No visual language model (VL model) detected" 或 "SQAI_MODEL_FAMILY is required" 错误，请确认已经正确配置 VL 模型的 `SQAI_MODEL_FAMILY` 环境变量。

从 1.0 版本开始，Midscene 推荐使用 `SQAI_MODEL_FAMILY` 来指定视觉模型类型。旧的 `MIDSCENE_USE_...` 配置仍然兼容但已废弃。
