...
export default defineConfig({
  testDir: './e2e',
+ timeout: 90 * 1000,
+ reporter: [["list"], ["@sqaitech/web/playwright-reporter", { type: "merged" }]],
});
```
...
当上面的命令执行成功后，会在控制台输出：`SQAI - report file updated: ./current_cwd/sqai_run/report/some_id.html`，通过浏览器打开该文件即可看到报告。
