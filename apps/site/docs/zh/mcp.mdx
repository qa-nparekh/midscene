...
`@sqaitech/web-bridge-mcp` 支持将 [Chrome 插件的桥接模式](./bridge-mode)发布为 MCP 服务。
...
在 MCP 客户端中添加 Midscene Web Bridge MCP 服务器 （ `@sqaitech/web-bridge-mcp` ）。其中模型配置的参数请参考 [模型策略](./model-strategy)。
...
```json
{
  "mcpServers": {
    "midscene-web": {
      ...
      "env": {
        "SQAI_MODEL_BASE_URL": "替换为你的模型服务地址",
        "SQAI_MODEL_API_KEY": "替换为你的 API Key",
        "SQAI_MODEL_NAME": "替换为你的模型名称",
        "SQAI_MODEL_FAMILY": "替换为你的模型系列",
...
在 MCP 客户端中添加 Midscene iOS MCP 服务器（ `@sqaitech/ios-mcp` ）。其中模型配置的参数请参考 [模型策略](./model-strategy)。
...
在 MCP 客户端中添加 Midscene Android MCP 服务器（ `@sqaitech/android-mcp` ）。其中模型配置的参数请参考 [模型策略](./model-strategy)。
...
`@sqaitech/computer-mcp` 支持将电脑桌面自动化能力发布为 MCP 服务，让 AI 可以通过鼠标、键盘和截图操作来控制你的电脑。
...
在 MCP 客户端中添加 Midscene Computer MCP 服务器（ `@sqaitech/computer-mcp` ）。其中模型配置的参数请参考 [模型策略](./model-strategy)。
