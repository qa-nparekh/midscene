...
各平台包（`@sqaitech/android`、`@sqaitech/ios`、`@sqaitech/web` 等）现在各自暴露了独立的 CLI 入口，Skills 正是基于此能力构建。
...
新增 `@sqaitech/computer-mcp` 包，将 PC 桌面自动化能力以独立 MCP 服务的形式提供。开发者可以直接在 Cursor、Trae 等支持 MCP 的工具中使用桌面自动化能力，无需额外集成。
...
- **动态配置**: 修复 Playground 中环境变量 `SQAI_REPLANNING_CYCLE_LIMIT` 未正确读取的问题
...
环境变量重命名（向后兼容）
- 改名 `OPENAI_API_KEY` → `SQAI_MODEL_API_KEY`（新变量优先，旧变量作为备选）
- 改名 `OPENAI_BASE_URL` → `SQAI_MODEL_BASE_URL`（新变量优先，旧变量作为备选）

### ⬆️ 升级到最新版

升级项目中的依赖，例如：

`npm install @sqaitech/web@latest --save-dev`
`npm install @sqaitech/android@latest --save-dev`
`npm install @sqaitech/ios@latest --save-dev`
...
#### 自定义重新规划循环限制
- 设置 `SQAI_REPLANNING_CYCLE_LIMIT` 环境变量，可以自定义在执行操作(aiAct)时允许的最大重新规划循环次数
- 默认值为 10，当 AI 需要重新规划超过这个限制时，会抛出错误建议将任务拆分
- 提供更灵活的任务执行控制，适应不同复杂度的自动化场景
```bash
export SQAI_REPLANNING_CYCLE_LIMIT=10 # 默认值为 10
```
...
npx @sqaitech/cli --headed --url https://www.baidu.com --action "输入 '天气', 敲回车" --sleep 3000
