// config keys
export const SQAI_MODEL_INIT_CONFIG_JSON =
  'SQAI_MODEL_INIT_CONFIG_JSON';
export const SQAI_MODEL_NAME = 'SQAI_MODEL_NAME';
export const SQAI_DEBUG_MODEL_PROFILE = 'SQAI_DEBUG_MODEL_PROFILE';
export const SQAI_DEBUG_MODEL_RESPONSE = 'SQAI_DEBUG_MODEL_RESPONSE';
export const SQAI_DANGEROUSLY_PRINT_ALL_CONFIG =
  'SQAI_DANGEROUSLY_PRINT_ALL_CONFIG';
export const SQAI_DEBUG_MODE = 'SQAI_DEBUG_MODE';
export const SQAI_MCP_USE_PUPPETEER_MODE =
  'SQAI_MCP_USE_PUPPETEER_MODE';
export const SQAI_MCP_CHROME_PATH = 'SQAI_MCP_CHROME_PATH';
export const SQAI_MCP_ANDROID_MODE = 'SQAI_MCP_ANDROID_MODE';
export const DOCKER_CONTAINER = 'DOCKER_CONTAINER';
export const SQAI_FORCE_DEEP_THINK = 'SQAI_FORCE_DEEP_THINK';

// Observability
export const SQAI_LANGSMITH_DEBUG = 'SQAI_LANGSMITH_DEBUG';
export const SQAI_LANGFUSE_DEBUG = 'SQAI_LANGFUSE_DEBUG';

export const SQAI_MODEL_SOCKS_PROXY = 'SQAI_MODEL_SOCKS_PROXY';
export const SQAI_MODEL_HTTP_PROXY = 'SQAI_MODEL_HTTP_PROXY';

// New primary names for public API
export const SQAI_MODEL_API_KEY = 'SQAI_MODEL_API_KEY';
export const SQAI_MODEL_BASE_URL = 'SQAI_MODEL_BASE_URL';
export const SQAI_MODEL_MAX_TOKENS = 'SQAI_MODEL_MAX_TOKENS';
export const SQAI_MODEL_TIMEOUT = 'SQAI_MODEL_TIMEOUT';
export const SQAI_MODEL_TEMPERATURE = 'SQAI_MODEL_TEMPERATURE';
export const SQAI_MODEL_RETRY_COUNT = 'SQAI_MODEL_RETRY_COUNT';
export const SQAI_MODEL_RETRY_INTERVAL = 'SQAI_MODEL_RETRY_INTERVAL';

/**
 * @deprecated Use SQAI_MODEL_API_KEY instead. This is kept for backward compatibility.
 */
export const OPENAI_API_KEY = 'OPENAI_API_KEY';
/**
 * @deprecated Use SQAI_MODEL_BASE_URL instead. This is kept for backward compatibility.
 */
export const OPENAI_BASE_URL = 'OPENAI_BASE_URL';
/**
 * @deprecated Use SQAI_MODEL_INIT_CONFIG_JSON instead. This is kept for backward compatibility.
 */
export const SQAI_OPENAI_INIT_CONFIG_JSON =
  'SQAI_OPENAI_INIT_CONFIG_JSON';
/**
 * @deprecated Use SQAI_MODEL_HTTP_PROXY instead. This is kept for backward compatibility.
 */
export const SQAI_OPENAI_HTTP_PROXY = 'SQAI_OPENAI_HTTP_PROXY';
/**
 * @deprecated Use SQAI_MODEL_SOCKS_PROXY instead. This is kept for backward compatibility.
 */
export const SQAI_OPENAI_SOCKS_PROXY = 'SQAI_OPENAI_SOCKS_PROXY';
/**
 * @deprecated Use SQAI_MODEL_MAX_TOKENS instead. This is kept for backward compatibility.
 */
export const OPENAI_MAX_TOKENS = 'OPENAI_MAX_TOKENS';

export const SQAI_ADB_PATH = 'SQAI_ADB_PATH';
export const SQAI_ADB_REMOTE_HOST = 'SQAI_ADB_REMOTE_HOST';
export const SQAI_ADB_REMOTE_PORT = 'SQAI_ADB_REMOTE_PORT';
export const SQAI_ANDROID_IME_STRATEGY = 'SQAI_ANDROID_IME_STRATEGY';

export const SQAI_IOS_DEVICE_UDID = 'SQAI_IOS_DEVICE_UDID';
export const SQAI_IOS_SIMULATOR_UDID = 'SQAI_IOS_SIMULATOR_UDID';

export const SQAI_CACHE = 'SQAI_CACHE';
export const SQAI_USE_VLM_UI_TARS = 'SQAI_USE_VLM_UI_TARS';
export const SQAI_USE_QWEN_VL = 'SQAI_USE_QWEN_VL';
export const SQAI_USE_QWEN3_VL = 'SQAI_USE_QWEN3_VL';
export const SQAI_USE_DOUBAO_VISION = 'SQAI_USE_DOUBAO_VISION';
export const SQAI_USE_GEMINI = 'SQAI_USE_GEMINI';
export const SQAI_USE_VL_MODEL = 'SQAI_USE_VL_MODEL';
export const MATCH_BY_POSITION = 'MATCH_BY_POSITION';
export const SQAI_REPORT_TAG_NAME = 'SQAI_REPORT_TAG_NAME';
export const SQAI_REPORT_QUIET = 'SQAI_REPORT_QUIET';

export const SQAI_PREFERRED_LANGUAGE = 'SQAI_PREFERRED_LANGUAGE';

export const SQAI_CACHE_MAX_FILENAME_LENGTH =
  'SQAI_CACHE_MAX_FILENAME_LENGTH';

export const SQAI_REPLANNING_CYCLE_LIMIT =
  'SQAI_REPLANNING_CYCLE_LIMIT';

export const SQAI_RUN_DIR = 'SQAI_RUN_DIR';

// INSIGHT (unified VQA and Grounding)
export const SQAI_INSIGHT_MODEL_NAME = 'SQAI_INSIGHT_MODEL_NAME';
export const SQAI_INSIGHT_MODEL_SOCKS_PROXY =
  'SQAI_INSIGHT_MODEL_SOCKS_PROXY';
export const SQAI_INSIGHT_MODEL_HTTP_PROXY =
  'SQAI_INSIGHT_MODEL_HTTP_PROXY';
export const SQAI_INSIGHT_MODEL_BASE_URL =
  'SQAI_INSIGHT_MODEL_BASE_URL';
export const SQAI_INSIGHT_MODEL_API_KEY = 'SQAI_INSIGHT_MODEL_API_KEY';
export const SQAI_INSIGHT_MODEL_INIT_CONFIG_JSON =
  'SQAI_INSIGHT_MODEL_INIT_CONFIG_JSON';
export const SQAI_INSIGHT_MODEL_TIMEOUT = 'SQAI_INSIGHT_MODEL_TIMEOUT';
export const SQAI_INSIGHT_MODEL_TEMPERATURE =
  'SQAI_INSIGHT_MODEL_TEMPERATURE';
export const SQAI_INSIGHT_MODEL_RETRY_COUNT =
  'SQAI_INSIGHT_MODEL_RETRY_COUNT';
export const SQAI_INSIGHT_MODEL_RETRY_INTERVAL =
  'SQAI_INSIGHT_MODEL_RETRY_INTERVAL';
export const SQAI_INSIGHT_MODEL_FAMILY = 'SQAI_INSIGHT_MODEL_FAMILY';

// PLANNING
export const SQAI_PLANNING_MODEL_NAME = 'SQAI_PLANNING_MODEL_NAME';
export const SQAI_PLANNING_MODEL_SOCKS_PROXY =
  'SQAI_PLANNING_MODEL_SOCKS_PROXY';
export const SQAI_PLANNING_MODEL_HTTP_PROXY =
  'SQAI_PLANNING_MODEL_HTTP_PROXY';
export const SQAI_PLANNING_MODEL_BASE_URL =
  'SQAI_PLANNING_MODEL_BASE_URL';
export const SQAI_PLANNING_MODEL_API_KEY =
  'SQAI_PLANNING_MODEL_API_KEY';
export const SQAI_PLANNING_MODEL_INIT_CONFIG_JSON =
  'SQAI_PLANNING_MODEL_INIT_CONFIG_JSON';
export const SQAI_PLANNING_MODEL_TIMEOUT =
  'SQAI_PLANNING_MODEL_TIMEOUT';
export const SQAI_PLANNING_MODEL_TEMPERATURE =
  'SQAI_PLANNING_MODEL_TEMPERATURE';
export const SQAI_PLANNING_MODEL_RETRY_COUNT =
  'SQAI_PLANNING_MODEL_RETRY_COUNT';
export const SQAI_PLANNING_MODEL_RETRY_INTERVAL =
  'SQAI_PLANNING_MODEL_RETRY_INTERVAL';
export const SQAI_PLANNING_MODEL_FAMILY = 'SQAI_PLANNING_MODEL_FAMILY';
export const SQAI_MODEL_FAMILY = 'SQAI_MODEL_FAMILY';

/**
 * env keys declared but unused
 */
export const UNUSED_ENV_KEYS = [SQAI_DANGEROUSLY_PRINT_ALL_CONFIG];

/**
 * env keys for debug or basic run
 * can not be override by overrideAIConfig
 */
export const BASIC_ENV_KEYS = [
  SQAI_DEBUG_MODE,
  SQAI_DEBUG_MODEL_PROFILE,
  SQAI_DEBUG_MODEL_RESPONSE,
  SQAI_RUN_DIR,
] as const;

export const BOOLEAN_ENV_KEYS = [
  SQAI_CACHE,
  SQAI_FORCE_DEEP_THINK,
  SQAI_MCP_USE_PUPPETEER_MODE,
  SQAI_MCP_ANDROID_MODE,
  SQAI_LANGSMITH_DEBUG,
  SQAI_LANGFUSE_DEBUG,
  SQAI_REPORT_QUIET,
] as const;

export const NUMBER_ENV_KEYS = [
  SQAI_MODEL_MAX_TOKENS,
  SQAI_CACHE_MAX_FILENAME_LENGTH,
  SQAI_REPLANNING_CYCLE_LIMIT,
] as const;

export const STRING_ENV_KEYS = [
  SQAI_MODEL_MAX_TOKENS,
  OPENAI_MAX_TOKENS,
  SQAI_ADB_PATH,
  SQAI_ADB_REMOTE_HOST,
  SQAI_ADB_REMOTE_PORT,
  SQAI_ANDROID_IME_STRATEGY,
  SQAI_IOS_DEVICE_UDID,
  SQAI_IOS_SIMULATOR_UDID,
  SQAI_REPORT_TAG_NAME,
  SQAI_PREFERRED_LANGUAGE,
  MATCH_BY_POSITION,
  SQAI_MCP_CHROME_PATH,
  DOCKER_CONTAINER,
] as const;

/**
 * Non model related env keys, used for globally controlling the behavior of sqai
 * Can not be override by agent.modelConfig but can be override by overrideAIConfig
 * Can be access at any time
 */
export const GLOBAL_ENV_KEYS = [
  ...BOOLEAN_ENV_KEYS,
  ...NUMBER_ENV_KEYS,
  ...STRING_ENV_KEYS,
] as const;

/**
 * Model related eve keys, used for declare which model to use.
 * Can be override by both agent.modelConfig and overrideAIConfig
 * Can only be access after agent.constructor
 */
export const MODEL_ENV_KEYS = [
  // model default
  SQAI_MODEL_NAME,
  SQAI_MODEL_INIT_CONFIG_JSON,
  SQAI_MODEL_API_KEY,
  SQAI_MODEL_BASE_URL,
  SQAI_MODEL_SOCKS_PROXY,
  SQAI_MODEL_HTTP_PROXY,
  SQAI_MODEL_TIMEOUT,
  SQAI_MODEL_TEMPERATURE,
  SQAI_MODEL_RETRY_COUNT,
  SQAI_MODEL_RETRY_INTERVAL,
  SQAI_USE_VLM_UI_TARS,
  SQAI_USE_QWEN_VL,
  SQAI_USE_QWEN3_VL,
  SQAI_USE_DOUBAO_VISION,
  SQAI_USE_GEMINI,
  SQAI_USE_VL_MODEL,
  // model default legacy
  OPENAI_API_KEY,
  OPENAI_BASE_URL,
  SQAI_OPENAI_INIT_CONFIG_JSON,
  SQAI_OPENAI_HTTP_PROXY,
  SQAI_OPENAI_SOCKS_PROXY,
  // INSIGHT (unified VQA and Grounding)
  SQAI_INSIGHT_MODEL_NAME,
  SQAI_INSIGHT_MODEL_SOCKS_PROXY,
  SQAI_INSIGHT_MODEL_HTTP_PROXY,
  SQAI_INSIGHT_MODEL_BASE_URL,
  SQAI_INSIGHT_MODEL_API_KEY,
  SQAI_INSIGHT_MODEL_INIT_CONFIG_JSON,
  SQAI_INSIGHT_MODEL_TIMEOUT,
  SQAI_INSIGHT_MODEL_TEMPERATURE,
  SQAI_INSIGHT_MODEL_RETRY_COUNT,
  SQAI_INSIGHT_MODEL_RETRY_INTERVAL,
  SQAI_INSIGHT_MODEL_FAMILY,
  // PLANNING
  SQAI_PLANNING_MODEL_NAME,
  SQAI_PLANNING_MODEL_SOCKS_PROXY,
  SQAI_PLANNING_MODEL_HTTP_PROXY,
  SQAI_PLANNING_MODEL_BASE_URL,
  SQAI_PLANNING_MODEL_API_KEY,
  SQAI_PLANNING_MODEL_INIT_CONFIG_JSON,
  SQAI_PLANNING_MODEL_TIMEOUT,
  SQAI_PLANNING_MODEL_TEMPERATURE,
  SQAI_PLANNING_MODEL_RETRY_COUNT,
  SQAI_PLANNING_MODEL_RETRY_INTERVAL,
  SQAI_PLANNING_MODEL_FAMILY,
  SQAI_MODEL_FAMILY,
] as const;

export const ALL_ENV_KEYS = [
  ...UNUSED_ENV_KEYS,
  ...BASIC_ENV_KEYS,
  ...GLOBAL_ENV_KEYS,
  ...MODEL_ENV_KEYS,
] as const;

export type TEnvKeys = (typeof ALL_ENV_KEYS)[number];
export type TGlobalConfig = Record<TEnvKeys, string | undefined>;

export type TVlModeValues =
  | 'qwen2.5-vl'
  | 'qwen3-vl'
  | 'qwen3.5'
  | 'doubao-vision'
  | 'doubao-seed'
  | 'gemini'
  | 'vlm-ui-tars'
  | 'vlm-ui-tars-doubao'
  | 'vlm-ui-tars-doubao-1.5'
  | 'glm-v'
  | 'auto-glm'
  | 'auto-glm-multilingual';

export const VL_MODE_RAW_VALID_VALUES: TVlModeValues[] = [
  'doubao-vision',
  'doubao-seed',
  'gemini',
  'qwen2.5-vl',
  'qwen3-vl',
  'qwen3.5',
  'vlm-ui-tars',
  'vlm-ui-tars-doubao',
  'vlm-ui-tars-doubao-1.5',
  'glm-v',
  'auto-glm',
  'auto-glm-multilingual',
];

/**
 * Model family values - unified model configuration approach
 * Replaces the old SQAI_USE_* environment variables
 *
 * Note: These values directly correspond to VL_MODE_RAW_VALID_VALUES
 * - 'qwen2.5-vl' is Qwen 2.5
 * - 'qwen3-vl' is Qwen 3
 * - 'qwen3.5' is Qwen 3.5 (behaves the same as qwen3-vl)
 */
export type TModelFamily = TVlModeValues | 'gpt-5';

export const MODEL_FAMILY_VALUES: TModelFamily[] = [
  ...VL_MODE_RAW_VALID_VALUES,
  'gpt-5',
];

export interface IModelConfigForInsight {
  // model name
  [SQAI_INSIGHT_MODEL_NAME]: string;
  // proxy
  [SQAI_INSIGHT_MODEL_SOCKS_PROXY]?: string;
  [SQAI_INSIGHT_MODEL_HTTP_PROXY]?: string;
  // OpenAI
  [SQAI_INSIGHT_MODEL_BASE_URL]?: string;
  [SQAI_INSIGHT_MODEL_API_KEY]?: string;
  [SQAI_INSIGHT_MODEL_INIT_CONFIG_JSON]?: string;
  // timeout
  [SQAI_INSIGHT_MODEL_TIMEOUT]?: string;
  // temperature
  [SQAI_INSIGHT_MODEL_TEMPERATURE]?: string;
  // model family
  [SQAI_INSIGHT_MODEL_FAMILY]?: TModelFamily;
}

export interface IModelConfigForPlanning {
  // model name
  [SQAI_PLANNING_MODEL_NAME]: string;
  // proxy
  [SQAI_PLANNING_MODEL_SOCKS_PROXY]?: string;
  [SQAI_PLANNING_MODEL_HTTP_PROXY]?: string;
  // OpenAI
  [SQAI_PLANNING_MODEL_BASE_URL]?: string;
  [SQAI_PLANNING_MODEL_API_KEY]?: string;
  [SQAI_PLANNING_MODEL_INIT_CONFIG_JSON]?: string;
  // timeout
  [SQAI_PLANNING_MODEL_TIMEOUT]?: string;
  // temperature
  [SQAI_PLANNING_MODEL_TEMPERATURE]?: string;
  // model family
  [SQAI_PLANNING_MODEL_FAMILY]?: TModelFamily;
}

/**
 * Model configuration for Planning intent.
 *
 * IMPORTANT: Planning MUST use a vision language model (VL mode).
 * DOM-based planning is not supported.
 *
 * Required: SQAI_MODEL_FAMILY must be set to one of:
 *   - 'qwen2.5-vl'
 *   - 'qwen3-vl'
 *   - 'qwen3.5'
 *   - 'gemini'
 *   - 'doubao-vision'
 *   - 'doubao-seed'
 *   - 'vlm-ui-tars'
 *   - 'vlm-ui-tars-doubao'
 *   - 'vlm-ui-tars-doubao-1.5'
 *   - 'glm-v'
 */
export interface IModelConfigForDefault {
  // model name
  [SQAI_MODEL_NAME]: string;
  // proxy
  [SQAI_MODEL_SOCKS_PROXY]?: string;
  [SQAI_MODEL_HTTP_PROXY]?: string;
  // OpenAI
  [SQAI_MODEL_BASE_URL]?: string;
  [SQAI_MODEL_API_KEY]?: string;
  [SQAI_MODEL_INIT_CONFIG_JSON]?: string;
  // extra
  [SQAI_MODEL_FAMILY]?: TModelFamily;
  // temperature
  [SQAI_MODEL_TEMPERATURE]?: string;
}

export interface IModelConfigForDefaultLegacy {
  // model name
  [SQAI_MODEL_NAME]: string;
  // proxy
  [SQAI_OPENAI_SOCKS_PROXY]?: string;
  [SQAI_OPENAI_HTTP_PROXY]?: string;
  // OpenAI
  [OPENAI_BASE_URL]?: string;
  [OPENAI_API_KEY]?: string;
  [SQAI_OPENAI_INIT_CONFIG_JSON]?: string;
}

/**
 * - insight: Visual Question Answering and Visual Grounding (unified)
 * - planning: planning
 * - default: all except insight、planning
 */
export type TIntent = 'insight' | 'planning' | 'default';

/**
 * Env-style model configuration map supplied directly to the agent.
 * Numbers are allowed so callers can pass numeric env values (e.g. limits) without casting.
 */
export type TModelConfig = Record<string, string | number>;

export enum UITarsModelVersion {
  V1_0 = '1.0',
  V1_5 = '1.5',
  DOUBAO_1_5_15B = 'doubao-1.5-15B',
  DOUBAO_1_5_20B = 'doubao-1.5-20B',
}

/**
 * Callback to create custom OpenAI client instance
 * @param config - Resolved model configuration including apiKey, baseURL, modelName, intent, etc.
 * @returns OpenAI client instance (can be wrapped with langsmith, langfuse, etc.)
 *
 * Note: Wrapper functions like langsmith's wrapOpenAI() return the same OpenAI instance
 * with enhanced behavior, so the return type remains compatible with OpenAI.
 *
 * Note: The return type is `any` in the shared package to avoid requiring openai as a dependency.
 * The actual implementation should return an OpenAI instance.
 *
 * @example
 * ```typescript
 * import OpenAI from 'openai';
 * import { wrapOpenAI } from 'langsmith/wrappers';
 *
 * createOpenAIClient: async (openai, opts) => {
 *   // Wrap with langsmith for planning tasks
 *   if (opts.baseURL?.includes('planning')) {
 *     return wrapOpenAI(openai, { metadata: { task: 'planning' } });
 *   }
 *
 *   return openai;
 * }
 * ```
 */
export type CreateOpenAIClientFn = (
  openAIInstance: any,
  options: Record<string, unknown>,
) => Promise<any>; // OpenAI instance, but typed as `any` to avoid dependency

export interface IModelConfig {
  /**
   * proxy
   */
  socksProxy?: string;
  httpProxy?: string;
  /**
   * model
   */
  modelName: string;
  /**
   * OpenAI
   */
  openaiBaseURL?: string;
  openaiApiKey?: string;
  openaiExtraConfig?: Record<string, unknown>;
  /**
   * Timeout for API calls in milliseconds.
   * If not set, uses OpenAI SDK default (10 minutes).
   */
  timeout?: number;
  /**
   * Temperature for model sampling.
   */
  temperature?: number;
  /**
   * Number of retries when AI call fails.
   * Default is 1 (retry once after failure).
   */
  retryCount?: number;
  /**
   * Interval between retries in milliseconds.
   * Default is 2000.
   */
  retryInterval?: number;
  /**
   * Model family - unified model configuration
   * Maps directly to model families like 'qwen2.5-vl', 'qwen3-vl', 'doubao-vision', 'doubao-seed', etc.
   */
  modelFamily?: TModelFamily;
  uiTarsModelVersion?: UITarsModelVersion;
  modelDescription: string;
  /**
   * original intent from the config
   */
  intent: TIntent;
  /**
   * Custom OpenAI client factory function
   *
   * If provided, this function will be called to create OpenAI client instances
   * for each AI call, allowing you to:
   * - Wrap clients with observability tools (langsmith, langfuse)
   * - Use custom OpenAI-compatible clients
   * - Apply different configurations based on intent
   */
  createOpenAIClient?: CreateOpenAIClientFn;
}
