...
describe('overrideAIConfig', () => {
  ...
  it('should accept valid MODEL_ENV_KEYS', () => {
    ...
    expect(() =>
      globalConfigManager.overrideAIConfig({
        ...
        [SQAI_MODEL_API_KEY]: 'sk-test-key',
  ...
  it('should merge config values in extend mode', () => {
    ...
    vi.stubEnv(SQAI_ADB_PATH, '/original/adb/path');
    vi.stubEnv(SQAI_CACHE, 'false');
    vi.stubEnv(SQAI_MODEL_MAX_TOKENS, '100');
...
describe('getEnvConfigValueAsNumber', () => {
  ...
  it('should return undefined for non-numeric string values', () => {
    vi.stubEnv(SQAI_ADB_PATH, 'not-a-number');

    const globalConfigManager = new GlobalConfigManager();
    globalConfigManager.registerModelConfigManager(new ModelConfigManager());

    expect(
      globalConfigManager.getEnvConfigValueAsNumber(SQAI_ADB_PATH),
    ).toBeUndefined();
  });
  ...
  it('should return undefined for empty string values', () => {
    vi.stubEnv(SQAI_MODEL_MAX_TOKENS, '');
  ...
  it('should handle decimal values', () => {
    vi.stubEnv(SQAI_MODEL_MAX_TOKENS, '123.45');
  ...
  it('should handle zero values', () => {
    vi.stubEnv(SQAI_MODEL_MAX_TOKENS, '0');
  ...
  it('should throw if key is not a supported numeric key', () => {
    ...
    expect(() =>
      globalConfigManager.getEnvConfigValueAsNumber(
        // @ts-expect-error SQAI_CACHE is truly not a valid string key (it's boolean)
        SQAI_CACHE,
      ),
    ).toThrowErrorMatchingInlineSnapshot(
      '[Error: getEnvConfigValueAsNumber with key SQAI_CACHE is not supported.]',
    );
  });
});
...
describe('getEnvConfigValue', () => {
  beforeEach(() => {
    ...
    // Reset SQAI_CACHE to ensure tests start with clean state
    vi.stubEnv(SQAI_CACHE, '');
  });
  ...
  it('should throw if key is not supported', () => {
    ...
    expect(() =>
      ...
      '[Error: getEnvConfigValueAsNumber with key SQAI_MODEL_NAME is not supported.]',
    );
