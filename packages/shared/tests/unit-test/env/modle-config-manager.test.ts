...
describe('ModelConfigManager', () => {
  ...
  const baseMap = {
    ...
    [SQAI_INSIGHT_MODEL_NAME]: 'gpt-4-vision',
    [SQAI_INSIGHT_MODEL_API_KEY]: 'insight-key',
    [SQAI_INSIGHT_MODEL_BASE_URL]: 'https://insight.openai.com/v1',
    [SQAI_PLANNING_MODEL_NAME]: 'qwen-vl-plus',
    [SQAI_PLANNING_MODEL_API_KEY]: 'plan-key',
    [SQAI_PLANNING_MODEL_BASE_URL]: 'https://plan.openai.com/v1',
  ...
  it('prefer SQAI_MODEL', () => {
    vi.stubEnv(SQAI_MODEL_NAME, 'env-model');
    vi.stubEnv(SQAI_MODEL_API_KEY, 'env-key');
    vi.stubEnv(SQAI_MODEL_BASE_URL, 'https://env.example.com');
    vi.stubEnv(OPENAI_API_KEY, 'openai-api-env-key');
    vi.stubEnv(OPENAI_BASE_URL, 'openai-base-url');
    vi.stubEnv(SQAI_MODEL_FAMILY, 'qwen3-vl');
  ...
  it('provides upload server URL from openaiExtraConfig', () => {
    const manager = new ModelConfigManager({
      ...baseMap,
      [SQAI_MODEL_INIT_CONFIG_JSON]: JSON.stringify({
  ...
  describe('per-intent timeout configuration', () => {
    it('uses per-intent timeout configs from modelConfig', () => {
      const configWithTimeout = {
        ...baseMap,
        [SQAI_MODEL_TIMEOUT]: '45000',
        [SQAI_INSIGHT_MODEL_TIMEOUT]: '60000',
        [SQAI_PLANNING_MODEL_TIMEOUT]: '90000',
      };
    ...
    it('reads per-intent timeout from environment variables', () => {
      ...
      vi.stubEnv(SQAI_MODEL_BASE_URL, 'https://env.example.com');
      vi.stubEnv(SQAI_MODEL_FAMILY, 'qwen3-vl');
      vi.stubEnv(SQAI_MODEL_TIMEOUT, '120000');
      vi.stubEnv(SQAI_INSIGHT_MODEL_NAME, 'insight-model');
      vi.stubEnv(
        SQAI_INSIGHT_MODEL_BASE_URL,
        'https://insight.example.com',
      );
      vi.stubEnv(SQAI_INSIGHT_MODEL_TIMEOUT, '180000');
      vi.stubEnv(SQAI_PLANNING_MODEL_NAME, 'planning-model');
      vi.stubEnv(
        SQAI_PLANNING_MODEL_BASE_URL,
        'https://planning.example.com',
      );
      vi.stubEnv(SQAI_PLANNING_MODEL_TIMEOUT, '240000');
