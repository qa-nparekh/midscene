...
describe('ComputerMCPServer HTTP mode', () => {
  let server: ComputerMCPServer;
  const testPort = 13582; // Use different port than other mcp packages
  const testHost = '127.0.0.1'; // Use IPv4 explicitly to avoid IPv6 issues in CI

  beforeAll(async () => {
    server = new ComputerMCPServer();
  });
    ...
    try {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        try {
          ...
          connected = true;
          console.log(
            `✓ Computer MCP server started and responding (attempt ${i + 1})`,
          );
          break;
        } catch (error) {
          if (i === 9) {
  ...
  it('should reject invalid port numbers', async () => {
    const invalidServer = new ComputerMCPServer();
